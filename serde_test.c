...
mu_test(generics_supported_and_correct) {
  ...
  NO_ERR(serde_do(write, mem, almost_two));
  ...
  NO_ERR(serde_do(read, mem, &got_almost_two));
...
mu_test(bytebuffer_supported_and_corect) {
  bytebuffer_t* mem = bb_alloc(1024);
